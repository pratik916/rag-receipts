...
def test_write_corpus_marshals_r1_layout_and_pinned_kwargs(tmp_path):
    ...
    def fake_run_ingest(*, corpus_id, data_dir, ingest_config, embed, qdrant):
        ...
        return {"corpus_id": corpus_id, "n_docs": 2, "index_hashes": {"sparse": "sha256:x"}}
    ...
    docs = [
        LoadedDoc(doc_id="a", text="alpha text", source_file="a.txt", split_index=0, n_splits=1),
        LoadedDoc(doc_id="b#part0", text="beta", source_file="b.md", split_index=0, n_splits=2),
    ]
    messages: list[str] = []
    manifest = sink.write_corpus(
        corpus_id="my-docs", docs=docs, emit=lambda msg, progress: messages.append(msg)
    )
    ...
    assert rows[0] == {"doc_id": "a", "passage_id": "a", "title": "a.txt", "text": "alpha text"}
